...
public class Aggregation {
    public static void main(String[] args) throws Exception {
        ...
        DataStream < String > data = env.readTextFile("input/avg1.txt");
        ...
        mapped.keyBy(t -> t.f0).sum(3).writeAsText("output/agg/sum.txt");

        mapped.keyBy(t -> t.f0).min(3).writeAsText("output/agg/min.txt");

        mapped.keyBy(t -> t.f0).minBy(3).writeAsText("output/agg/minBy.txt");

        mapped.keyBy(t -> t.f0).max(3).writeAsText("output/agg/max.txt");

        mapped.keyBy(t -> t.f0).maxBy(3).writeAsText("output/agg/maxBy.txt");
        // execute program
